...
function handleChallenge(crcToken, secret) {
  ...
  const responseToken = crypto.createHmac('sha256', secret).update(crcToken).digest('base64');
...
async function processTweetEvent(tweet) {
  try {
    ...
    const isMention = tweetText.includes(`@${myUsername}`) || tweet.in_reply_to_user_id_str === myUserId;
...
async function getWebhookStatus() {
  ...
  return {
    ...
    events: ['tweet.create (mentions)', 'direct_message_events', 'favorite_events (likes)', 'follow_events']
  };
}
