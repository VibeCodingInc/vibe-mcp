...
3. **Offer the moves as choices — only the relevant ones.** Use the native question control if this host has one. Claude Code: AskUserQuestion with one option per move (its label, with the evidence in the description) plus **"not now"** — at most four options; the person can always pick the built-in "Other" to write their own, so do not add a "write my own" option. If the tool returned one move, offer one move; never pad the list with contacts the tool did not name. Otherwise print them numbered, then "0 = write my own, n = not now", and ask for a number. Selecting sends nothing.
